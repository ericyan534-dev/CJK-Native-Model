...
class DataCollatorForWWM:
    ...
    def __call__(self, batch: List[str]) -> Dict[str, torch.Tensor]:
        tokenized = [self.tokenizer(list(text), is_split_into_words=True, return_tensors="pt", return_struct_ids=True) for text in batch]
        input_ids = torch.cat([item["input_ids"] for item in tokenized], dim=0)
        struct_ids = torch.cat([item["struct_ids"] for item in tokenized], dim=0)
