...
class TreeMLPEncoder(nn.Module):
    """Recursive Tree-MLP encoder for IDS trees."""
    ...
    def forward(self, struct_ids: torch.LongTensor) -> torch.Tensor:
        # struct_ids: (batch, seq)
        batch_shape = struct_ids.shape
        flat_ids = struct_ids.view(-1)
        unique_ids = flat_ids.unique()
        cache: Dict[int, torch.Tensor] = {}
        for sid in unique_ids.tolist():
            cache[sid] = self._encode_single(sid)
        stacked = torch.stack([cache[int(sid)] for sid in range(len(self.struct_index_to_char))])
        gathered = stacked[struct_ids]
        return gathered
...
class CNMBertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if struct_ids is None:
            ...
            struct_embed = self.tree_encoder(struct_ids)
        concat = torch.cat([embeddings, struct_embed], dim=-1)
        fused = self.fusion(concat)
        return self.dropout(self.LayerNorm(fused))
...
class CNMBertModel(BertModel):
    ...
    def forward(self, struct_ids: Optional[torch.LongTensor] = None, **kwargs):
        if "inputs_embeds" in kwargs:
            return super().forward(**kwargs)
        kwargs["struct_ids"] = struct_ids
        return super().forward(**kwargs)
...
class CNMForMaskedLM(BertForMaskedLM):
    ...
    def forward(self, struct_ids: Optional[torch.LongTensor] = None, **kwargs):
        return super().forward(struct_ids=struct_ids, **kwargs)
